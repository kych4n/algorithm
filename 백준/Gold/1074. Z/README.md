...
### 설계
<p>분할 정복으로 탐색 영역의 변의 길이가 1이고, 목표 지점을 탐색할 때까지 재귀 함수를 호출한다.</p>
<p>재귀 함수로 모든 영역을 탐색하면, 시간 초과가 발생한다. 따라서, 모든 영역을 탐색할 필요 없이 사분면 중 하나를 탐색한다.</p>
<p>단, 순서대로 진행했을 때, 탐색하지 않는 사분면의 크기만큼 결과에 더해주어야 한다.</p>
