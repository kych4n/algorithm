...
### 설계
예산의 상한액(최댓값)을 구해야 하기 때문에, 우선 오름차순 정렬을 한다.
앞에서부터 어떤 지방이 요청한 예산을 본인과 이후 지방에 상한액으로 할당해도 총 예산 이하라면, 그 예산을 상한액으로 설정한다.
그렇지 않다면, 남은 상한액에서 본인과 이후 지방의 개수만큼 나눠서 할당한다.
