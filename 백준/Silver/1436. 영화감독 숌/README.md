...
### 설계
<p>규칙은 있지만, 그 규칙을 적용하기에 너무 복잡하고, 구체적으로 몇 번째 수인지 구하기는 어렵다. 따라서, 666부터 수를 증가시키면서, 그 수 안에 666이 있으면, n을 감소시키고 0이 될 때까지 반복한다. N번째 수를 찾는 과정에서 666이 있든지 없든지 현재 수를 증가시켜야 하므로 마지막에 1을 감소시켜야 한다.</p>
