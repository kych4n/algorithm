...
### 설계
<p>마지막은 무조건 밟아야 하므로, 마지막에서 시작했다.</p>
<p>현재 밟은 계단까지 연속된 것이 1개면 first에, 2개면 second에 저장하고, 두 칸을 내려가면 두 칸 위의 first와 second 점수 중 큰 것과 더하고, 한 칸을 내려가면 3개를 연속해서 밟을 수는 없으므로, 한 칸 위의 second와는 더할 수 없어서 first와만 더해야 한다.</p>
<p>시작점에 도달하기까지는 연속 횟수에 포함시키지 않으므로, 첫 번째와 두 번째 계단에서 저장한 값 중 가장 큰 것을 반환해야 한다.</p>
