...
<p>
 <ol>
  <li>답이 될 수 있는 후보는 <u>1부터 랜선의 최대 길이까지</u>이다. 랜선의 최대 길이는 2^31-1이므로, 순차 탐색을 사용하면, 시간 초과가 발생할 것이다. 따라서, 이분 탐색을 사용한다.</li>
  <li>이분 탐색으로 구한 랜선의 길이로 현재 가지고 있는 k개의 랜선을 나눈다.</li>
  <li>나눴을 때 가지게 되는 랜선의 개수가 n보다 작다면, 랜선의 길이를 줄이고, 크거나 같다면, 가장 큰 랜선의 길이를 찾도록 랜선의 길이를 증가시키고 결과 값을 갱신한다.</li>
 </ol>
</p>
