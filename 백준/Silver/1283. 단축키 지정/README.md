...
### 설계
1. 각 줄마다 각 단어의 첫 번째가 지정되었는지 확인 후 지정 여부 결정 -> \n 단위로 공백을 받는 문자열이기 때문에 getline을 사용해야 해야 했습니다. getline을 사용하기 전에 cin >> n;을 했다면 cin에 \n이 남아 있으므로, cin.ignore()를 통해 초기화해야 했습니다.
2. 1에서 지정되지 않았을 때, 전체를 탐색하며 지정되었는지 확인 후 지정 여부 결정
