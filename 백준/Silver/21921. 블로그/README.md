...
### 설계
x 기간의 방문자 수의 합을 구하려면, 각 날의 누적 합에 대해 -x 날의 누적 합을 빼면 된다.
현재 가장 큰 기간의 방문자 수보다 크다면 갱신하고 그 기간의 수를 1로 설정하고, 그렇지 않으면 1을 더한다.
주의해야할 것은 i-x를 하기 위해 i>=x에서 실행할 경우, index 0을 포함하는 기간이 고려되지 않을 수 있으므로, 1부터 방문자 수를 받도록 했다.
